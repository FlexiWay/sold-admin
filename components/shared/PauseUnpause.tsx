          ...
          <div className={`${sold.tokenManager?.active ? `rounded-full p-2 bg-brand-secondary bg-opacity-20` : `rounded-full bg-black p-4`}`}>
            {
              !sold.tokenManager?.active ? <>
                {/* pause */}
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6 animate-pulse  cursor-pointer" onClick={sold.handleToggleActive}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 5.25v13.5m-7.5-13.5v13.5" />
                </svg>
              </> : <>
                {/* start */}
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="size-8   cursor-pointer bg-brand-secondary bg-opacity-40 rounded-full p-2 hover:scale-105 transition-all duration-300 ease-in-out" onClick={() => { sold.handleToggleActive }}>
