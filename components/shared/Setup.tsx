...
const Setup: React.FC = () => {
  ...
  const handleCreateTestQuoteMint = async () => {
    setLoading(true);
    try {
      const { mint, resCreateQuote } =
        await sold.createTestQuoteMint(setupOptions);
    ...
    <div className="w-full">
      <div className="w-full flex flex-col items-center justify-center gap-4 mb-8">
        <span className="text-xs opacity-50">
          Create a test mint and copy paste it into the quoteMint field of the
          form showing up after
        </span>
        <button className="secondaryCTA" onClick={handleCreateTestQuoteMint}>
          {loading && <Spin />}Create Test Quote Mint
        </button>
        {createQuoteMint && (
          <div
        ...
        <div className="w-full grid gird-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 p-8 bg-card-bg rounded-lg lg:rounded-xl text-center border border-white border-opacity-10">
          ...
          <label className="flex flex-col gap-2 items-start justify-start">
            ...
            <input
              type="text"
              name="baseMintName"
              value={setupOptions.baseMintName}
          ...
          <label className="flex flex-col gap-2 items-start justify-start">
            ...
            <input
              type="text"
              name="baseMintSymbol"
              value={setupOptions.baseMintSymbol}
              onChange={handleInputChange}
              className="input input-bordered w-full bg-transparent"
            />
          </label>
          <label className="flex flex-col gap-2 items-start justify-start">
            ...
            <input
              type="text"
              name="baseMintUri"
              value={setupOptions.baseMintUri}
              onChange={handleInputChange}
              className="input input-bordered w-full bg-transparent"
            />
          </label>
          ...
          <label className="flex flex-col gap-2 items-start justify-start">
            ...
            <input
              ...
              value={setupOptions.baseMintDecimals}
              onChange={handleInputChange}
              className="input input-bordered w-full bg-transparent"
            />
          </label>
          <label className="flex flex-col gap-2 items-start justify-start">
            ...
            <input
              type="number"
              name="xMintDecimals"
              value={setupOptions.xMintDecimals}
              onChange={handleInputChange}
              className="input input-bordered w-full bg-transparent"
            />
          </label>
          ...
          <label className="flex flex-col gap-2 items-start justify-start">
            ...
            <div className="flex items-center gap-2">
              <input
                type="number"
                name="exchangeRate"
                value={setupOptions.exchangeRate}
                onChange={handleInputChange}
                className="input input-bordered w-full bg-transparent"
              />
          ...
          <label className="flex flex-col gap-2 items-start justify-start">
            ...
            <div className="flex items-center gap-2">
              <input
                type="number"
                name="stakingInitialExchangeRate"
                value={setupOptions.stakingInitialExchangeRate}
                onChange={handleInputChange}
                className="input input-bordered w-full bg-transparent"
              />
              <Tooltip title="Includes quoteMintDecimals">
          ...
          <label className="flex flex-col gap-2 items-start justify-start">
            ...
            <textarea
              name="allowList"
              value={setupOptions.allowList.join(", ")}
              onChange={handleInputChange}
          ...
          <label className="flex flex-col gap-2 items-start justify-start">
            ...
            <input
              ...
              value={setupOptions.withdrawTimeLock}
              onChange={handleInputChange}
              className="input input-bordered w-full bg-transparent"
            />
          </label>
          <label className="flex flex-col gap-2 items-start justify-start">
            ...
            <input
              ...
              value={setupOptions.withdrawExecutionWindow}
              onChange={handleInputChange}
              className="input input-bordered w-full bg-transparent"
            />
          </label>
          <div className="w-full flex items-center justify-center md:col-span-2 lg:col-span-3">
            <button type="submit" className="secondaryCTA flex-grow">
              Initialize System
            </button>
          </div>
        </div>
