        ...
        <>
            <div className="w-full flex flex-col items-center justify-center gap-2 p-8 bg-card-bg rounded-lg lg:rounded-xl text-center border border-white border-opacity-10">
                <div className="w-full flex items-center justify-start">
                    <span className="text-xl font-black -mt-2">Stake Pool Owner {isValidPublicKey(sold.poolManager?.pendingOwner || "0") ? "(pending)" : ""}</span>
                </div>
                <div className="w-full max-w-md mx-auto">
                    <div className="w-full flex items-center justify-between gap-4 mt-4">
                        <span className="text-xs text-white truncate w-3/4">
                            {isValidPublicKey(sold.poolManager?.pendingOwner || "0") ? getPendingOwnerPubKey() || getOwnerPubKey() : getOwnerPubKey()}
                        </span>
                        ...
                        {isPendingOwner && <button className="btn btn-sm" onClick={() => sold.handleAcceptUpdatePoolOwner()}>
                            ACCEPT
                        </button>}
                    </div>
                </div>
