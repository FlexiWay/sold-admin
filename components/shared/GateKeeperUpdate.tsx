...
const GateKeeperModal = ({ open, setOpen }: any) => {
  ...
  const handleUpdateClick = async () => {
    try {
      ...
      if (Array.isArray(parsedValue) && parsedValue.every(item => typeof item === 'string' && item.length > 0 && isValidPublicKey(item))) {
      ...
      setError('Invalid input. Please enter a valid JSON string array with up to 5 valid Solana public keys, e.g., ["key1", "key2"].');
    }
  };
          ...
          <div className="w-full flex items-center justify-center gap-4 mt-4">
            <div className="w-full flex flex-col items-start justify-between gap-2 h-4 bg-gray-500 bg-opacity-50 overflow-y-scroll">
              {sold.gateKeepers.length > 0 && sold.gateKeepers.map((keeper, index) => (
                <span key={index} className="text-xs text-white truncate">{keeper.toBase58()}</span>
              ))}
            </div>
